...
/**
 * Created by liuliang on 2017/2/19.
 */
...
public class EmulatorCheckHelper {
    ...
    private static EmulatorCheckHelper sInstance;

    private Context mContext;

    private Boolean isEmulator;

    public static EmulatorCheckHelper getEmulatorCheckHelper() {
        if (sInstance == null) {
            synchronized (EmulatorCheckHelper.class) {
                if (sInstance == null) {
                    sInstance = new EmulatorCheckHelper(BaseApplication.getApplication());
                }
            }
        }
        return sInstance;
    ...
    private EmulatorCheckHelper(Context context) {
        mContext = context;
    ...
    public boolean isEmulator() {
        if (isEmulator == null) {
            isEmulator = checkBasic() || checkAdvanced() || checkPackageName();
        }
        return isEmulator;
    }
